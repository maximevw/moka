...
public class GanacheContainer<SELF extends GanacheContainer<SELF>> extends GenericContainer<SELF> {
    ...
    protected void mapGanacheAccount(final int index, final String address) {
        final TestingAccount alreadyMappedAccount = this.testingAccounts.get(index);
        if (alreadyMappedAccount != null) {
            this.testingAccounts.put(index, alreadyMappedAccount.toBuilder().address(address).build());
        } else {
			String alias = null;
    ...
    protected void mapGanachePrivateKey(final int index, final String privateKey) {
        final TestingAccount alreadyMappedAccount = this.testingAccounts.get(index);
        if (alreadyMappedAccount != null) {
            this.testingAccounts.put(index, alreadyMappedAccount.toBuilder().privateKey(privateKey).build());
        } else {
			String alias = null;
			if (index < this.accountsAliases.size()) {
				alias = this.accountsAliases.get(index);
    ...
    public Web3j getWeb3j() {
        if (this.web3j == null) {
            try {
                ...
                throw new RuntimeException("Unable to get Web3j instance.", e);
            }
        }
        return this.web3j;
    }
    ...
    public SELF withLockingPassphrase(final String passphrase) {
        ...
        if (StringUtils.isBlank(passphrase)) {
            throw new IllegalArgumentException("The locking passphrase cannot be null.");
        }
    ...
    public SELF withWalletHdPath(final String hdPath) {
        if (StringUtils.isBlank(hdPath)) {
            throw new IllegalArgumentException("The hierarchical deterministic path cannot be null.");
        }
    ...
    public SELF withDefaultTransactionGasLimit(final String transactionGasLimit) {
        ...
        if (StringUtils.isBlank(transactionGasLimit)) {
			...
			throw new IllegalArgumentException("The transaction gas limit must be an hexadecimal value or 'estimate'.");
		}
    ...
    public SELF withDifficulty(final String difficulty) {
        ...
        if (StringUtils.isBlank(difficulty)) {
			...
			throw new IllegalArgumentException("The difficulty must be an hexadecimal value.");
		}
    ...
    public SELF withCallGasLimit(final String gasLimit) {
        if (StringUtils.isBlank(gasLimit)) {
			...
			throw new IllegalArgumentException("The call gas limit must be an hexadecimal value.");
		}
